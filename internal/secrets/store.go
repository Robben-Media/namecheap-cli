...
import (
	...
	"github.com/builtbyrobben/namecheap-cli/internal/config"
)
...
const (
	apiKeyKey          = "api_key"
	keyringPasswordEnv = "NAMECHEAP_CLI_KEYRING_PASS" //nolint:gosec // env var name, not a credential
	keyringBackendEnv  = "NAMECHEAP_CLI_KEYRING_BACKEND"
...
func openKeyringWithTimeout(cfg keyring.Config, timeout time.Duration) (keyring.Keyring, error) {
	...
	case <-time.After(timeout):
		return nil, fmt.Errorf("%w after %v (D-Bus SecretService may be unresponsive); "+
			"set NAMECHEAP_CLI_KEYRING_BACKEND=file and NAMECHEAP_CLI_KEYRING_PASS=<password> to use encrypted file storage instead",
...
// DeleteSecret removes a generic secret by key.
